...
namespace AIRadio.Server.Services.Report
{
    public sealed class ReportTool : ITool
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var sections =
                request.GetArgument<List<string>>(
                    "sections")
                ?? [];
            ...
            if (sections.Count == 0)
            {
                return ToolResult.Failed(
                    Name,
                    "No report sections were specified.");
            }
            ...
            if (normalizedSections.Contains("weather") ||
                normalizedSections.Contains("news"))
            {
                if (location is null)
                {
                    return ToolResult.Failed(
        ...
        private async Task<NewsResult> GetNewsAsync(
            string? locationName,
            CancellationToken cancellationToken)
        {
            ...
            return await _newsService.GetHeadlinesAsync(
                query,
                cancellationToken);
        }

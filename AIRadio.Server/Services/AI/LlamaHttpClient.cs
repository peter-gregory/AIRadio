...
namespace AIRadio.Server.Services.AI
{
    public interface ILlamaHttpClient
    {
        ...
        Task<LlamaCompletionResponse> CompleteAsync(LlamaCompletionRequest request, CancellationToken cancellationToken = default);
    }
    ...
    public sealed class LlamaHttpClient : ILlamaHttpClient
    {
        ...
        public LlamaHttpClient(IConfiguration configuration, ILogger<LlamaHttpClient> logger, HttpClient httpClient)
        {
        ...
        public Task InitializeAsync(CancellationToken cancellationToken = default)
        {
            ...
            var endpoint = _configuration["Llama:Endpoint"];
            if (string.IsNullOrWhiteSpace(endpoint))
                throw new InvalidOperationException("Llama:Endpoint is not configured.");

            _endpoint = endpoint.TrimEnd('/');
            _initialized = true;
            _logger.LogInformation("Llama HTTP client initialized with endpoint {Endpoint}.", _endpoint);
            return Task.CompletedTask;
        }

        public async Task<LlamaCompletionResponse> CompleteAsync(LlamaCompletionRequest request, CancellationToken cancellationToken = default)
        {
            ThrowIfDisposed();
            ...
            var json = LlamaJsonOptions.Serialize(request);
        ...
        {
            if (!_initialized)
                throw new InvalidOperationException("LlamaHttpClient has not been initialized.");
        }
    ...
    public static class LlamaResponseSchema
    {
        public static JObject Create() => JObject.Parse("""
        {
          "title": "LlamaResponse",
          "type": "object",
          "properties": {
          ...
          "required": ["spokenText", "toolRequests", "soundEvents"],
          "additionalProperties": false
        }
        """);
    }

...
namespace AIRadio.Server.Services.Events
{
    public sealed class EventTool : ITool
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var timestamp =
                request.GetArgument<DateTime?>("timestamp");

            var includeAlarms =
                request.GetBoolean("includeAlarms")
                ?? true;

            var includeReminders =
                request.GetBoolean("includeReminders")
                ?? true;

            var events =
                _alarmManager.GetEvents(timestamp);

            var filtered =
                events
                    .Where(eventItem =>
                        (includeAlarms &&
                         eventItem.Type == ScheduledEventType.Alarm) ||
                        (includeReminders &&
                         eventItem.Type == ScheduledEventType.Reminder))
                    .ToList();
        ...
        private static EventReportItem MapEvent(

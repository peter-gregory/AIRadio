...
namespace AIRadio.Server.Services.Audio
{
    ...
    public sealed class AudioManager : IAudioManager, IAsyncDisposable
    {
        ...
        private async Task DuckMpvAsync(CancellationToken cancellationToken)
        {
            ...
            // A volume of zero represents an intentionally muted MPV.
            // Do not treat an already-muted player as ducked, otherwise
            // UnduckMpvAsync would later restore the volume and unexpectedly
            // unmute the radio.
            if (_normalVolume == 0)
            {
                _logger.LogDebug("MPV radio is already muted; skipping duck.");
                return;
            }
        ...
        private async Task UnduckMpvAsync(CancellationToken cancellationToken)
        {
            ...
            // If MPV is muted now, preserve that mute state rather than
            // restoring the volume captured before ducking.
            var currentVolume = await _mpvClient.GetVolumeAsync(cancellationToken);
            if (currentVolume == 0)
            {
                Volatile.Write(ref _isDucked, false);
                _logger.LogDebug("MPV radio is muted; skipping unduck volume restore.");
                return;
            }
